pipeline {
    ...
    stages {
        ...
        stage('Pulling base image from Dockerhub') {
            steps {
                    sh 'docker pull 5ggraduationproject/udr-base'
            }
        }
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t 5ggraduationproject/5g-udr:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image 5ggraduationproject/5g-udr --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push 5ggraduationproject/5g-udr:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi 5ggraduationproject/5g-udr'
            sh 'docker rmi 5ggraduationproject/udr-base'
        }
    }
}